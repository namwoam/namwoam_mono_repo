...
{
    size_t biggest = 0;
    for (int i = 1; i < NumericArray<T>::Size(); i++)
    {
        if (repr_.At(biggest) < repr_.At(i))
        {
            biggest = i;
        }
    }
    return repr_[biggest];
}
...
T NumericArray<T>::Min() const
{
    ...
    for (int i = 1; i < NumericArray<T>::Size(); i++)
    {
        if (repr_.At(smallest) > repr_.At(i))
        {
            smallest = i;
        }
    }
    return repr_[smallest];
}
...
{
    T total = NumericArray<T>::Size();
    return NumericArray<T>::Sum() / total;
}
...
void NumericArray<T>::TrimZeroes()
{
    ...
    while (NumericArray<T>::Size() != 0)
    {
        if (repr_.At(NumericArray<T>::Size() - 1) == zero)
        {
            repr_.PopBack();
        }
        else
